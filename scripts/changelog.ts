...
export async function getChangeLog(count = 200) {
  ...
  const r = (
    await execCommand('git', ['log', '-n', `${count}`, `--pretty=format:%H|%ad|%s|%D|%b|%an|%ae`])
  )
    .split('\n')
    .map((v) => {
  ...
  for (const log of logs) {
    ...
    const raw = await execCommand('git', [
      'diff-tree',
      '--no-commit-id',
      '--name-only',
      '-r',
...
export async function getContributorsAt(path: string) {
  try {
    const list = (await execCommand('git', ['log', '--pretty=format:%an|%ae', '--', path]))
      .split('\n')
